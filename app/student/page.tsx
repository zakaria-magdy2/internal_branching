...
function sortProgramsBySavedOrder(
...
export default function StudentPreferencesPage() {
  ...
  const [registrationStateReason, setRegistrationStateReason] = useState<
  ...
  const loadStudentData = async (ssn: string) => {
    ...
    try {
      ...
      try {
        ...
        if (checkRes.ok) {
          ...
          try {
            ...
            else if (typeof checkJson?.data === "boolean")
              studentCanReg = checkJson.data;
            else if (checkJson?.canRegister !== undefined)
              studentCanReg = Boolean(checkJson.canRegister);
            else if (checkJson?.isAllowed !== undefined)
              studentCanReg = Boolean(checkJson.isAllowed);
      ...
      try {
        const resultRes = await apiCall(
          `/api/Student/get-student-result/${ssn}`,
        );
        if (resultRes.ok) {
      ...
      try {
        const progRes = await apiCall(
          `/api/Student/get-available-programs/${ssn}`,
        );
        if (progRes.ok) {
          const data = await progRes.json().catch(() => null);
          const list = Array.isArray(data)
            ? data
            : data?.$values || data?.data || data?.programs || [];
      ...
      if (savedOrderFromLocal.length > 0 && fetchedPrograms.length > 0) {
        fetchedPrograms = sortProgramsBySavedOrder(
          fetchedPrograms,
          savedOrderFromLocal,
        );
      }
    ...
    try {
      ...
      if (!isError) {
        ...
        if (session?.ssn) {
          try {
            await apiCall(
              `/api/Student/${encodeURIComponent(session.ssn.trim())}/phone-number`,
              {
                method: "PUT",
        ...
        try {
          if (session?.ssn) {
            localStorage.setItem(
              `student_prefs_${session.ssn}`,
              JSON.stringify(programs.map((p) => p.name)),
            );
        ...
        if (
          cleanText.toLowerCase().includes("user not found") ||
          cleanText.includes("غير موجود")
        ) {
          ...
          displayError =
            "فترة تسجيل الرغبات مغلقة حالياً في النظام من قبل إدارة الكلية.";
        }
        showToast(
          `لم يتم الحفظ: ${displayError || "حدث خطأ في النظام"} ❌`,
          "error",
        );
      }
    } catch {
    ...
    return (
      ...
      >
        ...
        <h3 style={{ color: "var(--text-main)", fontSize: "1.1rem" }}>
          جاري تحميل بيانات الطالب والبرامج...
        </h3>
      </div>
    );
  }
    ...
    >
      ...
      <header className="app-header">
        <div className="header-brand">
          ...
          />
          <div>
            <div className="header-title" style={{ fontWeight: 800 }}>
              كلية العلوم – جامعة قناة السويس
            </div>
            <div style={{ fontSize: "0.8rem", color: "var(--text-muted)" }}>
              منظومة التنسيق الداخلي للفرقة الأولى
            </div>
          </div>
        </div>
      ...
      <main className="student-container">
        ...
        <section className="student-info-banner">
          ...
          <div className="student-info-pills">
            ...
            <span className="pill-badge">عدد البرامج: {programs.length}</span>
          </div>
        </section>
        ...
        {studentResult ? (
          <div
            style={{
              background:
                "linear-gradient(135deg, rgba(16, 185, 129, 0.2) 0%, rgba(6, 95, 70, 0.35) 100%)",
        ...
        {registrationStateReason === "not_started" && !studentResult && (
          ...
          >
            فترة تسجيل الرغبات لم تبدأ بعد لهذا الحساب. يمكنك الاطلاع على
            البرامج المتاحة لحين فتح باب التسجيل.
          </div>
        )}
        ...
        {isSubmitted && !isReadOnly && (
          ...
          >
            تم حفظ وتأكيد رغباتك بنجاح في النظام ✅ يمكنك تعديل ترتيب الرغبات
            والضغط على "تعديل وحفظ ترتيب الرغبات" في أي وقت طالما باب التسجيل
            مفتوح.
          </div>
        )}

        {/* Instructions Card: تعليمات ترتيب الرغبات وتنبيه البرامج الخاصة */}
        <section
          className="preferences-instructions"
          style={{
            marginBottom: "1.5rem",
            display: "flex",
            flexDirection: "column",
        ...
        >
          ...
          >
            اذا كنت ترغب في برنامج خاص بمصروفات يرجى التوجهه الي شؤون الطلاب
            بالكليه وعدم التسجيل علي الموقع
          </div>
        </section>
            ...
            {displayed.map((program) => {
              ...
              const isAcceptedChoice = Boolean(
                ...
                (program.name.trim().toLowerCase() ===
                  studentResult.trim().toLowerCase() ||
                  studentResult
                    .trim()
                    .toLowerCase()
                    .includes(program.name.trim().toLowerCase()) ||
                  program.name
                    .trim()
                    .toLowerCase()
                    .includes(studentResult.trim().toLowerCase())),
              );
              ...
              return (
                <div
                  ...
                  style={{
                    borderColor: isAcceptedChoice
                      ? "#10b981"
                      : isHighlight
                        ? "var(--accent)"
                        : "var(--border-color)",
                    background: isAcceptedChoice
                      ? "rgba(16, 185, 129, 0.12)"
                      : isHighlight
                        ? "rgba(0, 212, 184, 0.06)"
                        : "var(--surface-card)",
                    boxShadow: isAcceptedChoice
                ...
                >
                  ...
                  <div className="pref-card-controls">
                    <select
                      ...
                      value={rank}
                      onChange={(e) =>
                        handleRankChange(realIdx, Number(e.target.value))
                      }
                      disabled={isReadOnly}
                    >
                      {programs.map((_, i) => (
        ...
        {programs.length > 0 &&
          (isReadOnly ? (
            ...
            <button
              type="button"
              className="btn-submit-preferences"
              onClick={() => setShowPhoneModal(true)}
            >
              {isSubmitted
                ? "تعديل وحفظ ترتيب الرغبات"
                : "حفظ وتأكيد ترتيب الرغبات"}
            </button>
          ))}
      </main>
      ...
      {showPhoneModal && (
        <div className="modal-overlay" onClick={() => setShowPhoneModal(false)}>
          ...
          >
            ...
            >
              يرجى إدخال رقم هاتفك المحمول لتأكيد اعتماد الترتيب الحالي في
              النظام.
            </p>
            ...
            >
              ...
              >
                معاينة الرغبات الأولى:
              </div>
              {programs.slice(0, 3).map((p, i) => (
                <div
                  key={p.id}
                  style={{
                    color: i === 0 ? "var(--accent)" : "var(--text-main)",
                    marginBottom: 4,
                  }}
                >
